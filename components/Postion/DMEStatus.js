...
const DMEStatus = ({ navigation }) => {
    ...
    const [DataYr, setDataYr] = useState([
        ...
        { label: 'EDL', value: '1' },
        { label: 'Non-EDL', value: '2' },
        { label: 'Above 1 Lakh Indent Value', value: '3' },
        { label: 'Below 1 Lakh Indent Value', value: '4' },
        { label: 'Return for LP', value: '5' },
    ...
    const fetchMCTransaction = async () => {
        try {
            ...
            if (segmentValue == "0")
            {             
               SetTitalValue("C.F.Y. DME Indent Status :Overall Items" );
            }
            if (covstatus == "1")
            {
      
               SetTitalValue("C.F.Y. DME Indent Status :EDL Items" );
            }
            if (covstatus == "2")
            {    
                SetTitalValue("C.F.Y. DME Indent Status :Non EDL Items" );
            }
            if (covstatus == "3")
            {          
                SetTitalValue("C.F.Y. DME Indent Status :Above 1 Lakh Indent Value" );
            }
            if (covstatus == "4")
            {          
                SetTitalValue("C.F.Y. DME Indent Status :Below 1 Lakh Indent Value" );
            }
            if (covstatus == "5")
            {          
                SetTitalValue("C.F.Y. DME Indent Status :Returned for LP" );
            }

           // alert("mcid"+ segmentValue);
            const resultData = await fetchDHSIndentvsIssuance(segmentValue,"2")
            setData(resultData);
        } catch (error) {
            console.error('Error:', error);
        }
