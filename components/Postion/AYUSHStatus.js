...
const AYUSHStatus = ({ navigation }) => {
    ...
    const [DataYr, setDataYr] = useState([
        ...
        { label: 'Ayurvedic Classical', value: '1' },
        { label: 'Ayurvedic Patent', value: '2' },
        { label: 'Homeo Classical', value: '3' },
        { label: 'Homeo Patent', value: '4' },
        { label: 'Unani Classical', value: '5' },
        { label: 'Unani Patent', value: '6' },
        { label: 'Crude Items', value: '7' },
        { label: 'Dressing Material', value: '8' },
    ...
    const fetchMCTransaction = async () => {
        try {
            ...
            if (segmentValue == "0")
            {
               ...
               SetTitalValue("C.F.Y. AYUSH Indent Status :Overall Items" );
            }
            if (covstatus == "1")
            {
      
               SetTitalValue("C.F.Y. AYUSH Indent Status :Ayurvedic Classical" );
            }
            if (covstatus == "2")
            {
    
                SetTitalValue("C.F.Y. AYUSH Indent Status :Ayurvedic Patent" );
            }
            if (covstatus == "3")
            {
          
                SetTitalValue("C.F.Y. AYUSH Indent Status :Homeo Classical" );
            }
            if (covstatus == "4")
            {
          
                SetTitalValue("C.F.Y. AYUSH Indent Status :Homeo Patent" );
            }
            if (covstatus == "5")
            {
          
                SetTitalValue("C.F.Y. AYUSH Indent Status :Unani Classical" );
            }
            if (covstatus == "6")
            {
          
                SetTitalValue("C.F.Y. AYUSH Indent Status :Unani Patent" );
            }
            if (covstatus == "7")
            {
          
                SetTitalValue("C.F.Y. AYUSH Indent Status :Crude Items" );
            }
            if (covstatus == "8")
            {
          
                SetTitalValue("C.F.Y. AYUSH Indent Status :Dressing Materials" );
            }

           // alert("mcid"+ segmentValue);
            const resultData = await fetchDHSIndentvsIssuance(segmentValue,"3")
            setData(resultData);
        } catch (error) {
            console.error('Error:', error);
        }
