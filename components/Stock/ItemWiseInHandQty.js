...
import { DdlItemWiseInHandQty, ItemWiseInHandQtyDetail, fetchCGMSCStockQTY, fetchPublicStockitems, fetchdhsDmeYearConsumption, fetchgetDhsDmeStock, fetchitemIndentQuantity } from '../Services/apiService';
...
const ItemWiseInHandQty = ({ navigation }) => {
  ...
  const [districtid, setDistrictid] = useState(informaitonAboutUser.districtid);
  ...
  const fetchData = async () => {
    ...
    try {
      setVisible(true);
      //alert("value:" + value);
      const response = await ItemWiseInHandQtyDetail(value, districtid);
      setData(response);
    } catch (error) {
  ...
  const fetchDataWithMCAt = async () => {
    ...
    if (value == null) {
      //alert("Please Select Category")
      let toast = Toast.show('Please Select Item', {
        duration: Toast.durations.LONG,
        position: Toast.positions.BOTTOM,
      });
  ...
  const showItemDetails = () => {
    const selectedItem = dataDDL.find(item => item.itemid === value);
  ...
  const fetchMainCategory = async () => {
    try {
      ...
      const stockReportDataDDL = await DdlItemWiseInHandQty(districtid);
  ...
  const renderItem = ({ item, index }) => {
    return (
      ...
      >
        ...
        <View style={styles.cell}>
          <Text style={[styles.cellText, { textAlign: 'right' }]}>{item.inhanD_QTY}</Text>
        </View>
      </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <View>
        <Title style={styles.header1}>Item Detail</Title>
      </View>
        ...
        {itemDetails && (
          <View style={styles.detailsContainer}>
            ...
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detailValue}>{itemDetails.detail}</Text>
            </View>

            <View style={styles.detailRow}>
              <Text style={styles.detailLabel}>Code:</Text>
              <Text style={styles.detailValue}>{itemDetails.itemcode}</Text>
            </View>
            ...
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detailValue}>{itemDetails.inhanD_QTY}</Text>
            </View>
...
export default ItemWiseInHandQty;
