...
import { fetchDistFACwiseStockPostion, fetchDistrictDDL, fetchDistwiseIssuance, fetchFacilityMC, fetchFacwiseIssuance, fetchMainCategoryService, fetchMasitems, fetchPublicStockitems, ftechItemWHStock } from '../Services/apiService';
...
const IssuanceToFacilityOrHospital = ({ navigation }) => {
    ...
    const [dataDDLFacility, setDataDDLFacility] = useState([]);
    const [openFac, setOpenFac] = useState(false);
    const [valueFac, setValueFac] = useState(null);
    const [oldValueFac, setOldValueFac] = useState();
    ...
    const fetchData = async () => {
       // alert('valueItem' + valueItem);
        ...
        //alert('segmentValue' + segmentValue);
        ...
        //alert('valueFac' + valueFac);
        if (valueFac == 0 || valueFac == null) {
            alert("Please Select Facility")
            let toast = Toast.show('Please Select Facility', {
                duration: Toast.durations.LONG,
            });
            return;
        }
       // return;
        ...
        try {
            setVisible(true);
            fetchItemDetail();
            await fetchFACwiseIssuanceData();
    ...
    const fetchFACwiseIssuanceData = async () => {
        try {
            //SetTitalValue("Stock vs Issuance");
            //alert("inside fetchFACwiseIssuanceData" );
            let fromDate = 0;
            let toDate = 0;
            if (text != null) {
                fromDate = text;
            ...
            const response = await fetchFacwiseIssuance(valueYr, '0', segmentValue, "0", "0", valueFac);
            setData(response);
            //alert(data);
        } catch (error) {
            console.error('Error:', error);
    ...
    const fetchFacilityMCData = async () => {
        try {
            // const ddlData = await fetchDistrictDDL(tfvalud,Whid,"0",userid,approle);
            //alert("valueitem: " + valueItem);
            const ddlData = await fetchFacilityMC("0", "0", "0", "0", "0");
            setDataDDLFacility(ddlData);
        } catch (error) {
            console.error('Error:', error);
        }
    };
    ...
    const renderItem = ({ item, index }) => (
        ...
        >
            ...
            <View style={styles.cell}>
                <Text style={styles.cellText}>{item.indentdate}</Text>
            </View>
    ...
    return (
        <View style={styles.container}>
            ...
            <Appbar.Header style={styles.headerApp}>
                ...
                <Appbar.Content title="Issuance to Health Facility" titleStyle={styles.headerTitle} />
            </Appbar.Header>
            ...
            <View style={{ height: 40, width: '90%', margin: 20, zIndex: 1000 }}
            >
                <SegmentedButtons
                    value={segmentValue}
            ...
            {dataDDLFacility.length > 0 ? (
                <DropDownPicker
                    open={openFac}
                    value={valueFac}
                    searchable={true}
                    items={dataDDLFacility.map((item) => (
                        {
                    ...
                    onChangeValue={(value) => {
                        if (value != null) {
                            setOldValueFac(value);
                            // alert("outside mcid:"+value);
                            // alert("outside oldValueItem:"+oldValueItem);
                            if (value != oldValueFac && value != undefined) {
                                //alert("ddl invoked: value" + value + "old value: " + oldValue);
                                //fetchData();
                                // alert("mcid:"+value);
                                // alert("oldValueItem:"+oldValueItem);
            ...
            <View style={styles.header}>
                ...
                <Text style={styles.headerText}>Issued Date</Text>
...
export default IssuanceToFacilityOrHospital;
