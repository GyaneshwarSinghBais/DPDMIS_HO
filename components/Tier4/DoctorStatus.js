...
import { DrCountTotalData, fetchAllDoctor, fetchDistFACwiseStockPostion, fetchDistrictDDL, fetchDistwiseIssuance, fetchFacilityMC, fetchFacilityMCUser, fetchMainCategoryService, fetchMasitems, fetchOPDCountTotal, fetchPublicStockitems, ftechItemWHStock } from '../Services/apiService';
...
const DoctorStatus = ({ navigation }) => {
    ...
    const [dataDDLFac, setDataDDLFac] = useState([]);
    ...
    const [dataDDLDoc, setDataDDLDoc] = useState([]);
    const [openDoc, setOpenDoc] = useState(false);
    const [valueDoc, setValueDoc] = useState(null);
    const [oldValueDoc, setOldValueDoc] = useState();
    ...
    const fetchData = async () => {
        ...
        if (valueItem == 0 || valueItem == null) {
            ...
            let toast = Toast.show('Please Select Time Duration', {
        ...
        //     let toast = Toast.show('Please Select Summary/Detail', {
        //         duration: Toast.durations.LONG,
        //     });
        //     return;
        // }
    ...
    const fetchDistwiseIssuanceData = async () => {
        try {
            ...
            const response = await DrCountTotalData(distId, 'Collector', userid, valueYr, '0', valueItem, "0", valueDoc);
            setData(response);
            //alert(data);
        } catch (error) {
            console.error('Error:', error);
    ...
    //DDL datas
    const fetchFacilityMCData = async () => {
        try {
            // const ddlData = await fetchDistrictDDL(tfvalud,Whid,"0",userid,approle);
            //alert("valueitem: " + valueItem);
            const ddlData = await fetchFacilityMCUser('0', '0', '0', '0', '0',userid,'Collector');
            setDataDDLFac(ddlData);
        } catch (error) {
            console.error('Error:', error);
        }
    };
    ...
    const fetchAllDoctorData = async () => {
        try {
            // const ddlData = await fetchDistrictDDL(tfvalud,Whid,"0",userid,approle);
            //alert("valueitem: " + valueItem);
            const ddlData = await fetchAllDoctor("True",userid,id,"0",roleid);
            setDataDDLDoc(ddlData);
        } catch (error) {
            console.error('Error:', error);
        }
    };
    ...
    const renderItem = ({ item, index }) => (
        ...
        >
            ...
            <View style={styles.cell}>
                <Text style={styles.cellText}>{item.drname}</Text>
            </View>
        
            <View style={styles.cell}>
                <Text style={styles.cellText}>{item.noitemid}</Text>
            </View>
            <View style={styles.cell}>
                <Text style={styles.cellText}>{item.patno}</Text>
            </View>
    ...
    return (
        <View style={styles.container}>
            ...
            {/* <Appbar.Header style={styles.headerApp}>
                ...
                <Appbar.Content title="Doctor's Patient OPD" titleStyle={styles.headerTitle} />
            </Appbar.Header> */}
            ...
            {/* <View style={{ height: 40, width: '90%', margin: 20,marginBottom: 3, zIndex: 1000 }}
            >
                <SegmentedButtons
                    value={segmentValue}
            ...
            {dataDDLDoc.length > 0 ? (
                    <DropDownPicker
                        ...
                        onChangeValue={(value) => {
                            if (value != null) {
                                setOldValueDoc(value);
                                if (value != oldValueDoc && oldValueDoc != undefined) {
                                }
            ...
            <View style={styles.header}>
                ...
                <Text style={styles.headerText}>No's Item</Text>
                <Text style={styles.headerText}>No's Patient</Text>
