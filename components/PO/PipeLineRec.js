...
import { fetchInTransitProgress, ftechPipelinePODetails, ftechPipelinePOs, ftechPipelinePOsTransit, insertTblRecvProgress } from '../Services/apiService';
import DropDownPicker from 'react-native-dropdown-picker';
import { Card, Title, Paragraph, ActivityIndicator, MD2Colors, Dialog, Portal, TextInput } from 'react-native-paper';
   ...
   // const [TitalValue, SetTitalValue] = useState(null);
    ...
    const fetchGroupNameData = async () => {
        try {
            ...
            const res = await fetchInTransitProgress(id, roleid);
            setDataDDLGrp(res);
        } catch (error) {
    ...
    const fetchDataDDL = async () => {
        ...
        try {
            ...
            // alert(facuserid);
            const ddlData = await ftechPipelinePOsTransit("0", facuserid, userid)
            setDataDDL(ddlData);
        } catch (error) {
            console.error('Error:', error);
        }
    ...
    const insertTblRecvProgressData = async () => {
        try {
           // alert('save button clicked');
            const resultData = await insertTblRecvProgress(valueItem, textRemark, value, id);
            setData(resultData);
                ...
                <Card.Content>
                    <View style={styles.row}>
                        <View style={styles.column}>
                            ...
                            <Title style={styles.header}>Code:{item.itemcode}/PO:{item.pono}/Dated:{item.soissuedate}</Title>


                            <View style={styles.rowItem}>
                                <Title style={styles.label}>Previous Action:</Title>
                                <Paragraph style={styles.value}>{item.progress} Remarks:{item.remarks}/Date:{item.entrydate}</Paragraph>
                            </View>
                            <View style={styles.rowItem}>
                            ...
                            <View style={styles.rowItem}>
                                <Title style={styles.label}>2.Item:</Title>
                                <Paragraph style={styles.value}>{item.itemname}</Paragraph>
                            </View>

                            <View style={styles.rowItem}>
                                <Title style={styles.label}>3.Strength:</Title>
                                <Paragraph style={styles.value}>{item.strengtH1}</Paragraph>
                            </View>


                            <View style={styles.rowItem}>
                                <Title style={styles.label}>4.SKU:</Title>
                                <Paragraph style={styles.value}>{item.unit}</Paragraph>
                            </View>
                            <View style={styles.rowItem}>
                                <Title style={styles.label}>5.NIB Required:</Title>
                                <Paragraph style={styles.value}>{item.nablreq}</Paragraph>
                            </View>
                            <View style={styles.rowItem}>
                                <Title style={styles.label}>6.Ordered QTY:</Title>
                                <Paragraph style={styles.value}>{item.absqty}</Paragraph>
                            </View>
                            <View style={styles.rowItem}>
                                <Title style={styles.label}>7.Received QTY/%:</Title>
                                <Paragraph style={styles.value}>{item.receiptabsqty}/{item.recper}%</Paragraph>
                            </View>

                            <View style={styles.rowItem}>
                                <Title style={styles.label}>8.Pipeline QTY:</Title>
                                <Paragraph style={styles.value}>{item.pipelineqty}</Paragraph>
                            </View>
                            <View style={styles.rowItem}>
                                <Title style={styles.label}>9.Expected Delivery till:</Title>
                                <Paragraph style={styles.value}>{item.expecteddeliverydate}</Paragraph>
                            </View>

                            <View style={styles.rowItem}>
                                <Title style={styles.label}>10.Dispatched QTY by Supplier:</Title>
                                <Paragraph style={styles.value}>{item.disqty}</Paragraph>
                            </View>
                            <View style={styles.rowItem}>
                                <Title style={styles.label}>11.Supplier:</Title>
                                <Paragraph style={styles.value}>{item.suppliername}</Paragraph>
                            </View>
                            <View style={styles.rowItem}>
                                <Title style={styles.label}>12.Contact:</Title>
    ...
    return (
        ...
        <View style={styles.container}>
            ...
            <View style={{ marginLeft: 20, marginRight: 20, marginTop: 5,width: '90%'}}>              
                ...
                <TextInput
                    ...
                    value={textRemark}
                    onChangeText={setTextRemark}
                    style={styles.input}
                    left={<TextInput.Icon icon="fountain-pen-tip" />}
                />
            </View>


            <View style={StyleSheet.flatten([{ justifyContent: 'space-between', flexDirection: 'row', zIndex: 1000, width: '20%' }])} >
                <TouchableOpacity style={StyleSheet.flatten([styles.button, { marginTop: 10, marginBottom: 10, marginLeft: '50%', marginRight: '50%' }])} onPress={insertTblRecvProgressData}>
